...
> Tick source: subscribe to ROS topic `/bt/<proj>/bb/tick_id`, not screen-scrape rqt and not read JSONL for the status bar.
...
```text
1. rqt tree view and /bt/<proj>/bb/tick_id are both driven by the same BT runtime.
...
ROSA Agent Docker Container (host network)
  ...
  └── app.py — FastAPI page backend (port 8090)
      ├── rospy subscriber: /bt/<proj>/bb/tick_id
    ...
    <div id="layout">
      ...
      <div id="status-bar">
        ...
        <div class="status-cell">
          source: <span id="tick-source">/bt/<proj>/bb/tick_id</span>
        </div>
        <div class="status-cell">
...
3. Subscribe to `/bt/<proj>/bb/tick_id`.
...
/bt/<proj>/bb/tick_id stays at the paused tick
...
```text
BT node not running
ProjectBBBridge not publishing /bt/<proj>/bb/tick_id
...
docker exec rosa-agent bash -c \
  'source /opt/ros/noetic/setup.bash && rostopic echo -n 1 /bt/<proj>/bb/tick_id'
```
...
```text
1. Add active leaf to status bar by subscribing /<proj>_bt/log/tree or /<proj>_bt/ascii/snapshot.
2. Add BT mode by subscribing /<proj>_bt/bt/mode.

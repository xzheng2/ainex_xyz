...
Threshold/calibration source:
  CONFIG_DEFAULTS:
    center_x_offset: 66  (YAML-backed; xyz_bt_edu/config/line_perception.yaml
...
def _load_line_perception_config() -> dict:
    try:
        pkg_path = rospkg.RosPack().get_path('xyz_bt_edu')
...
class LineDetectionAdapter(XyzInputAdapter):
    ...
    def __init__(self, lock: threading.Lock, logger=None, tick_id_getter=None):
        ...
        center_x_offset is loaded from xyz_bt_edu/config/line_perception.yaml

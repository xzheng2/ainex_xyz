# 技术规格说明：xyz_bt_edu — AiNex 机器人行为树标准节点库
...
`xyz_bt_edu` 是 AiNex 人形机器人的 **BT 标准节点库**，供上层项目包（如 `xyz_behavior/marathon`）直接导入。
...
└── src/xyz_bt_edu/
    ├── __init__.py
    ├── base_facade.py            ← XyzBTFacade 抽象接口（项目 Facade 必须继承）
    ├── base_node.py              ← XyzBTNode 基类（所有节点继承）
    ├── blackboard_keys.py        ← BB key 唯一真实来源 + ROSA topic 映射表
    ├── bb_ros_bridge.py          ← BlackboardROSBridge（BB→ROS topic 镜像）
    ├── bt_runner.py              ← XyzBTRunner（生命周期管理）
...
> `setup.py` 中已声明 `xyz_bt_edu.input_adapters` 包，直接 import 即可，无需复制到项目。
...
> **唯一真实来源**：`xyz_bt_edu/blackboard_keys.py` — 所有 BB key 名称均在此定义，禁止在其他文件中硬编码字符串。
...
当 `xyz_bt_edu.behaviours/` 中**没有通用等价节点**时，项目可在自己的 `behaviours/` 目录下新增节点。
...
1. **必须继承 `XyzBTNode`** — 禁止直接继承 `py_trees.behaviour.Behaviour`
...
class LX_Module_Action(XyzBTNode):
    LEVEL = 'LX'
    BB_LOG_KEYS = [BB.ROBOT_STATE]   # 绝对路径常量 BB.*

    def __init__(self,
                 name: str = 'LX_Module_Action',
                 facade: XyzBTFacade = None,   # L2 节点必须；L1 可省略
...
```bash
# 在容器内构建
docker exec ainex bash -c "cd /home/ubuntu/ros_ws && catkin build xyz_bt_edu xyz_behavior"
...
python3 -c '
import sys, rospkg
sys.path.insert(0, rospkg.RosPack().get_path(\"xyz_behavior\"))
...
from xyz_bt_edu.behaviours.L2_locomotion.L2_Head_FindLineSweep import L2_Head_FindLineSweep
...
| v2.0.0 | 2026-04-15 | **Facade 重构**：引入 `XyzBTFacade` 抽象接口；节点与 ROS I/O 完全解耦；BB 改用 `/latched/` 命名空间；新增 `L2_Gait_FindLine` + `L2_Head_FindLineSweep`；36 节点中仅 7 个完成重构（其余存入 `behaviours_tem/` 待后续迁移） |
...
| v2.2.0 | 2026-04-17 | **L1_Head_IsHeadCentered 升级**：从 marathon 专属节点提升为标准库节点；继承 `XyzBTNode`；支持 `logger` + `tick_id_getter` 可观测性；marathon `conditions.py` 保留同名别名；`bb_ros_bridge.py` 新增 `/head_pan_pos` 镜像 |
...
| **v2.3.0** | **2026-04-17** | **`input_adapters/` 层正式化**：`ImuBalanceStateAdapter` + `LineDetectionAdapter` 迁移至 `xyz_bt_edu/input_adapters/`（`setup.py` 声明包）；两阶段 latch 协议（`snapshot_and_reset` under lock / `write_snapshot` after lock）正式成为框架规范；`rospy.Subscriber` 唯一合法位置约束写入设计原则；项目专属节点分工规范化：条件节点 → `behaviours/conditions.py`，动作节点 → `behaviours/actions.py`；debug log 归因链约束（`ros_out` 仅由 `comm_facade._emit()` 负责）和 `algorithms/` 纯函数约束写入文档 |

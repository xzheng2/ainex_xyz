...
class VisualPatrolNode(Common):
    ...
    def __init__(self, name):
        ...
        self.head_tilt_init = 340  # 上下舵机的初始值(initial value of the up-down servo)
        ...
        rospy.Subscriber('/fall_rise/state', String, self.state_callback)
    ...
    def run(self):
        while self.running:
            # rospy.loginfo_throttle(1, f"[Marathon] Robot state: {self.robot_state}")
            if self.start:
                if self.robot_state == 'stand':
                    ...
                    rospy.loginfo_throttle(1, f"Robot not in stand state, current: {self.robot_state}")
            else:
                time.sleep(0.01)

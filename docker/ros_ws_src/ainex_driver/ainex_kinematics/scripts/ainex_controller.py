...
class Controller:
    ...
    def __init__(self, name):
        ...
        for ctl_name, ctl_params in items_:
            if ctl_params['type'] == 'JointPositionController':
                ...
                coef = -self.ENCODER_TICKS_PER_RADIAN if flipped else self.ENCODER_TICKS_PER_RADIAN
                lo = min(min_angle_raw, max_angle_raw)
                hi = max(min_angle_raw, max_angle_raw)
                self.joint_angles_convert_coef[ctl_params['servo']['id']] = [initial_position_raw, coef, lo, hi]
    ...
    def angle2pulse(self, id_, angle):
        ...
        pulse = self.joint_angles_convert_coef[id_][0] + int(round(angle * self.joint_angles_convert_coef[id_][1]))
        lo, hi = self.joint_angles_convert_coef[id_][2], self.joint_angles_convert_coef[id_][3]
        return max(lo, min(pulse, hi))

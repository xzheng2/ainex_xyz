...
class PacketFunction(enum.IntEnum):
    # 可通过串口实现的控制功能(Control functions that can be implemented through serial port)
    PACKET_FUNC_SYS = 0
    ...
    def get_imu(self):
        if self.enable_recv:
            try:
                ...
                if len(data) == 4*9: # 9 axis ax, ay, az, gx, gy, gz, mx, my, mz
                    return struct.unpack('<9f', data)
                elif len(data) == 4*6: # 6 axis ax, ay, az, gx, gy, gz
...
def bus_servo_test(board):
    ...
    time.sleep(1)
    board.bus_servo_set_position(1, [[1, 0], [2, 0]])
    time.sleep(1)
...
if __name__ == "__main__":
    ...
    # board.bus_servo_set_angle_limit(13, [0, 1000])
    # print(board.bus_servo_read_angle_limit(13))
    # board.set_motor_speed([[1, -0.3], [2, 0.3], [3, -0.3], [4, 0.3]])
    # time.sleep(1)
    # board.set_rgb([[1, 255, 0, 255]])

...
class ROSRobotController:
    ...
    def __init__(self, name):
        ...
        self.servo_limits = None  # loaded lazily from /ainex_controller/controllers
        ...
        state_servo_ids = rospy.get_param('~state_servo_ids', list(range(1, 25)))
        state_rate_hz   = rospy.get_param('~state_rate_hz', 50.0)
        self._state_servo_ids = list(state_servo_ids)
        self._state_pub = rospy.Publisher('~bus_servo/state', SetBusServosPosition, queue_size=1)
        rospy.Timer(rospy.Duration(1.0 / state_rate_hz), self._pub_servo_state)
    ...
    def set_bus_servo_position(self, msg):
        # Lazy-load per-servo limits from ainex_controller params
        if self.servo_limits is None:
            limits = {}
            try:
                for ctl in rospy.get_param('/ainex_controller/controllers', {}).values():
                    if isinstance(ctl, dict) and ctl.get('type') == 'JointPositionController':
                        s = ctl.get('servo', {})
                        sid = s.get('id')
                        if sid is not None:
                            mn, mx = s.get('min', 0), s.get('max', 1000)
                            limits[sid] = (min(mn, mx), max(mn, mx))
            except Exception:
                pass
            self.servo_limits = limits  # empty dict = no clamping if params unavailable
        data = []
        for i in msg.position:
            pos = i.position
            if i.id in self.servo_limits:
                lo, hi = self.servo_limits[i.id]
                pos = max(lo, min(pos, hi))
            data.append([i.id, pos])
        if data:
    ...
    def _pub_servo_state(self, _event):
        msg = SetBusServosPosition()
        msg.duration = 0.0
        for sid in self._state_servo_ids:
            pos = self.board.servo_position.get(str(sid))
            if pos is None:
                continue
            entry = BusServoPosition()
            entry.id = sid
            entry.position = pos
            msg.position.append(entry)
        if msg.position:

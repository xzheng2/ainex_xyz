...
本包没有集中式 spec 文档。**每个模块的 docstring 就是权威规格**：
- 节点（`behaviours/L1_*/L2_*/L3_*`）：BB reads/writes、判定/策略规则、CONFIG_DEFAULTS
- 适配器（`adapters/`）：topic、消息类型、BB writes、提取/判别规则、两阶段 latch
- 组合工厂与 memory 实测规则：`core/composites.py`
- 稳定确认装饰器（含适用范围与已知缺口）：`core/latched_dwell.py`
- facade 接口：`core/base_facade.py`；节点基类契约：`core/base_node.py`
- BB 键（含废弃 key 清单）：`blackboard/blackboard_keys.py`

## 目录结构

```
core/          基类 / facade / 组合工厂 / latched_dwell / 渲染 / runner
...
```
docker exec -u ubuntu ainex bash -lc "cd ~/ros_ws && catkin build xyz_bt_lib"
docker exec -u ubuntu ainex bash -lc "source ~/ros_ws/devel/setup.bash && python3 -c 'import xyz_bt_lib'"
```

---

下面是三类组件的伪代码模板（结构速查；生成新文件用对应 skill 的 .tpl 模板）。
...
INPUT: CONFIG_DEFAULTS                          # 例：expected_state='stand', threshold=30
INPUT: logger/tick_id_getter                    # 只用于 decision 日志

__init__()                                      # 初始化条件节点
  inherit XyzL1ConditionNode                    # 让 BTDebugVisitor / node event 正常识别
...
update()                                        # 每次 tick 执行
  ...
  status = SUCCESS if passed else FAILURE       # L1 只返回 SUCCESS/FAILURE（纯谓词）
  emit_decision() if logger exists              # 例：inputs={'robot_state': 'stand'}
  OUTPUT: status                                # 给 BT tree selector/sequence 使用

_evaluate(value)                                # 无副作用判别函数
  compare with CONFIG_DEFAULTS values           # 例：state == self._expected_state
  OUTPUT: (passed, reason)                      # 例：(True, "robot_state == stand")

OUTPUT: Status.SUCCESS / Status.FAILURE         # 不写 BB，不调用 facade，不发 ROS
OUTPUT: optional decision event                 # 只走 emit_decision，不走 emit_comm
---

3. L2 Node 模板

INPUT: RUNTIME DATA: BB values                 # 每 tick 读取的数据
INPUT: CONFIG: CONFIG_DEFAULTS                 # 项目树实例化时覆盖的参数
INPUT: DEPENDENCY: facade                      # 外部注入的动作派发接口
INPUT: DEPENDENCY: logger/tick_id_getter       # 外部注入的观测与 tick 归因


__init__()                                      # 初始化动作/策略节点
  inherit XyzL2ActionNode                       # 标准 BT 节点身份
...
initialise()                                    # 节点从 IDLE 进入 RUNNING 时调用
  emit_action_intent() if useful                # 例：准备开始 follow_line
...
OUTPUT: optional BB write                       # 仅限 docstring 声明的动作状态/协调 key
OUTPUT: optional action_intent/decision         # 只走 emit_action_intent/emit_decision

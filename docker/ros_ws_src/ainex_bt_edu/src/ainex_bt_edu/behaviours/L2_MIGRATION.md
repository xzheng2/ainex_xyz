...
| Fix | Before | After |
|---|---|---|
| Base class | `AinexBTNode` | `AinexL1ConditionNode` |
...
1. `from ainex_bt_edu.base_node import AinexBTNode` → must be `AinexL2ActionNode`
2. `class L2_xxx(AinexBTNode)` → `class L2_xxx(AinexL2ActionNode)`
...
## `AinexL2ActionNode.__init__` Signature

```python
super().__init__(name, logger=logger, tick_id_getter=tick_id_getter, facade=facade)
```

`facade` is the injected `AinexBTFacade` implementation. All project trees
...
- `L2_Head_FindLineSweep` is the riskiest edit. Read the full tick-interruption
  fix pattern in `ainex_bt_edu.md` (memory) before touching it.
- After editing each file, run the import check inside the container:
  ...
  docker exec ainex bash -c "source /home/ubuntu/ros_ws/devel/setup.bash && \
    python3 -c 'from ainex_bt_edu.behaviours.L2_locomotion.<ClassName> import <ClassName>; print(\"OK\")'"
  ```
- Run the full compliance grep after finishing all 6:
  ```bash
  grep -rn "AinexBTNode\|emit_bt\b\|self\._logger\s*=" \
    docker/ros_ws_src/ainex_bt_edu/src/ainex_bt_edu/behaviours/L2_locomotion/
  ```

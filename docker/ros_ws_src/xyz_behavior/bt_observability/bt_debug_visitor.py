...
Records:
  ...
  - ros_topology_snapshot                 once at startup, after a settle delay (daemon thread)
"""
import time
import threading
import py_trees
...
class BTDebugVisitor(py_trees.visitors.VisitorBase):
    ...
    def __init__(self, logger, tick_id_getter):
        ...
        # Topology snapshot is captured exactly once per session, shortly after
        # startup. The settle delay lets every other node in the launch finish
        # registering with the master so the single retained snapshot is complete
        # (firing on tick 1 risks a partial graph). Topology is static during a run.
        self._topology_snapshot_done = False
        self._start_monotonic = time.monotonic()
        self._topology_settle_secs = 2.0

        # Enable py_trees built-in blackboard activity stream.
        # Note: if already enabled (e.g. by the project bt_node), this call may be
    ...
    def on_tree_tick_end(self, tree):
        ...
        # 3. topology snapshot: once per session, after a settle delay, in a daemon
        #    thread. Topology is static during a run, so a single startup capture is
        #    enough for post-mortem use. Async because snapshot_ros_topology can take
        #    5-10 s and must not block the 15 Hz tick loop.
        if (not self._topology_snapshot_done
                and time.monotonic() - self._start_monotonic >= self._topology_settle_secs):
            t = threading.Thread(
            ...
            t.start()
            self._topology_snapshot_done = True
    ...
    def _safe_repr(value):
        ...
        if hasattr(value, "__slots__"):          # ROS message object
            from bt_observability.ros_comm_tracer import _msg_to_dict
            return _msg_to_dict(value)
        return str(value)

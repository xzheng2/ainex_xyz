...
WHY THIS EXISTS
    ...
    All the real work lives in ``expt_run_lab/run_lab/run_context.py``, which the
...
_RUN_LAB_DIR = os.path.join(_WS_SRC, 'expt_run_lab')
...
def main(argv=None):
    ...
    # The node adopts this directory only if it sees one of these; without them, it opens
    # a run of its own and this one stays behind holding nothing but its meta.
    #
    # AINEX_RUN_DIR carries the RESOLVED absolute directory, and is what the generated
    # bt_node reads. Handing over the answer rather than the id is what lets the
    # scaffold stay free of expt_run_lab: a project generated by the skill needs a
    # directory to write into, not the rule for computing one. AINEX_RUN_ID is kept
    # because open_run_dir() still honours it and other tools consume it.
    sys.stderr.write('hand this run to the robot with:\n'
                     '  export AINEX_RUN_DIR={}\n'
                     '  export AINEX_RUN_ID={}\n'.format(run_dir, run_id))
    print(run_dir)
    return 0

...
> 最后更新：2026-04-17
> **完整规则同步维护于 skill**: `~/.claude/skills/xyz-bt-project/references/bt_observability_rules.md`
...
2. **通信边界三类**：`business_out`（唯一出口：`comm/comm_facade.py`）、
   `business_in`（唯一出口：`xyz_bt_edu/input_adapters/`，`app/` 层禁止 `rospy.Subscriber` 和 `ros_in` emit）、
   `infra`（写入 `infra_comm_manifest_lastrun.json`，不进 `bt_ros_comm_debug`）

3. **节点组合约束**：`tree/` 优先 import `xyz_bt_edu` 标准节点；项目节点继承
   `XyzBTNode`；`semantic_facade.py` 继承 `XyzBTFacade`；节点不直接调用 rospy
...
```
xyz_bt_edu/input_adapters/        ← rospy.Subscriber 唯一合法位置
...
每个传感器对应一个 `xyz_bt_edu/input_adapters/` 中的 Adapter 类。
...
| `xyz_bt_edu/input_adapters/` | business_in（`ros_in` + `input_state`） | 传感器输入适配；`app/` 层禁止直接 emit |
...
| 完整规则（12 节）| `~/.claude/skills/xyz-bt-project/references/bt_observability_rules.md` |

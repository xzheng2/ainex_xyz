...
> 最后更新：2026-04-17
> **完整规则同步维护于 skill**: `~/.claude/skills/xyz-bt-project/references/bt_infra_manifest_rules.md`
...
4. **节点组合约束**（与 bt_observability_framework.md 一致）：
   `package.xml` 必须声明 `<exec_depend>xyz_bt_edu</exec_depend>`
...
| subscriber **callback 收到消息**的采样事件（每 tick 记一次） | business_in | `xyz_bt_edu/input_adapters/` → `bt_ros_comm_debug_*.jsonl`（`ros_in` / `input_state`） |
...
> subscriber callback 实际收到的消息事件属于 business_in，由 `xyz_bt_edu/input_adapters/` 的 `write_snapshot()` 以 `ros_in` / `input_state` 写入 `bt_ros_comm_debug`，不在本清单记录。
...
- `infra_manifest.py`：`xyz_behavior/marathon/infra/infra_manifest.py`
- node 集成示例：`xyz_behavior/marathon/app/marathon_bt_node.py`，`__init__()` 末尾
- 完整规则（10 节，含验证命令）：`~/.claude/skills/xyz-bt-project/references/bt_infra_manifest_rules.md`
- 新项目脚手架：`/xyz-bt-project <project_name>` (Claude Code skill)

...
`xyz_behavior` BT 开发环境中生效的 Claude Code 扩展：`xyz-bt-project` skill + BT节点守护 hooks。

---

## 1. Skill — `xyz-bt-project`

**文件**: `~/.claude/skills/xyz-bt-project/SKILL.md`
...
1. 先查 `xyz_bt_edu/` 是否已有等价节点
2. 若无，生成 `behaviours/` 新节点（遵循 XyzBTNode 规范）
...
```python
from xyz_bt_edu.base import XyzBTNode

class MyNode(XyzBTNode):       # ① 继承 XyzBTNode
...
两个 Claude Code hook 守护 `xyz_behavior/.../behaviours/*.py` 的写入行为。
...
**路径匹配**: `xyz_behavior/.+/behaviours/[^/]+\.py`

行为：路径匹配时向 Claude 注入 skill 使用提醒，提示在写代码前先调用
`xyz-bt-project` skill 获取规范模板。不阻断写入（exit 0）。
...
| 检查项 | 规则 |
|---|---|
| ① XyzBTNode | `class Foo(XyzBTNode)` 模式存在 |
...
```bash
# 准备测试目录
mkdir -p /tmp/xyz_behavior/marathon/behaviours

# ① Pre hook 有提醒
echo '{"tool_name":"Write","tool_input":{"file_path":"/tmp/xyz_behavior/marathon/behaviours/test.py"}}' \
  | python3 ~/.claude/hooks/bt_node_pre_guard.py

# ② Post hook 非合规 → 违规列表
echo 'class Foo: pass' > /tmp/xyz_behavior/marathon/behaviours/bad.py
echo '{"tool_name":"Write","tool_input":{"file_path":"/tmp/xyz_behavior/marathon/behaviours/bad.py"}}' \
  | python3 ~/.claude/hooks/bt_node_guard.py

# ③ Post hook 合规 → 无输出
cat > /tmp/xyz_behavior/marathon/behaviours/good.py << 'EOF'
from xyz_bt_edu.base import XyzBTNode
class GoodNode(XyzBTNode):
    LEVEL = 'L2'
    BB_LOG_KEYS = ['robot_state']
    def update(self):
        return self.SUCCESS
EOF
echo '{"tool_name":"Write","tool_input":{"file_path":"/tmp/xyz_behavior/marathon/behaviours/good.py"}}' \
...
| 文件 | 说明 |
|---|---|
| `~/.claude/skills/xyz-bt-project/SKILL.md` | Skill 主文件（触发条件 + 完整工作流） |
| `~/.claude/skills/xyz-bt-project/references/` | 规范文档（observability + infra manifest rules） |
| `~/.claude/skills/xyz-bt-project/assets/templates/` | 代码模板（`*.py.tpl`） |

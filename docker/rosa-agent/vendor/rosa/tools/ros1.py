...
# Hard cap on the text size of a single echoed message. A raw 640x480 camera
# frame would otherwise render as ~3.7M chars (~920k tokens) and exceed the
# LLM context in one tool observation.
_ECHO_MAX_CHARS = 4000


def _safe_echo_text(msg, max_chars: int = _ECHO_MAX_CHARS) -> str:
    """
    Render a ROS message for LLM consumption, suppressing huge payload fields
    (e.g. sensor_msgs/Image.data) while keeping header/metadata readable.
    The message is a deserialized local copy and is never re-published, so
    mutating its fields here is safe.
    """
    suppressed = []
    for field in getattr(msg, "__slots__", []):
        try:
            ...
            continue
        if isinstance(value, (bytes, str, list, tuple)) and len(value) > max_chars:
            # Empty the field with a type-compatible value (genpy str() would
            # render a placeholder string in a uint8[] field as ord codes).
            setattr(msg, field, b"" if isinstance(value, bytes) else type(value)())
            suppressed.append(
                f"{field}: <{len(value)} bytes suppressed — use rqt_image_view "
                f"or a dedicated tool for raw data>"
            )
    text = str(msg)
    if suppressed:
        text += "\n" + "\n".join(suppressed)
...
def rostopic_echo(
    ...
    if not msg_class:
        # Distinguish "nobody publishes this topic" from "message package
        # missing in this container" — the generic error misleads the LLM
        # into suspecting access restrictions.
        try:
            master = rosgraph.Master("/rostopic_echo_check")
            publishers = {t for t, _ in master.getSystemState()[0]}
            if topic not in publishers:
                return {
                    "error": f"Topic '{topic}' has no active publisher — the node "
                    f"that publishes it is probably not running. Use rostopic_list "
                    f"to see live topics."
                }
            topic_type = dict(master.getTopicTypes()).get(topic, "unknown")
            return {
                "error": f"Topic '{topic}' is published (type {topic_type}), but "
                f"its message package is not installed in the agent container, "
                f"so it cannot be echoed here."
            }
        except Exception:
    ...
    for i in range(count):
        try:
            ...
            if return_echoes:
                msgs.append(_safe_echo_text(msg))
...
def rosservice_list(
    ...
    # When include_nodes=True, items are [service_name, [nodes]] instead of
    # plain strings.  Extract the service name for filtering.
    def _name(x):
        return x[0] if isinstance(x, (list, tuple)) else x

    if exclude_logging:
        services = list(filter(lambda x: not _name(x).startswith("/rosout"), services))

        # Exclude if the word "logger" is in the service name
        services = list(filter(lambda x: "logger" not in _name(x), services))

    if exclude_rosapi:
        services = list(filter(lambda x: not _name(x).startswith("/rosapi"), services))

    if exclude_parameters:
        services = list(filter(lambda x: "param" not in _name(x), services))

    if exclude_pattern:
        services = list(
            filter(lambda x: not regex.match(f".*{exclude_pattern}", _name(x)), services)
        )

    if regex_pattern:
        services = list(
            filter(lambda x: regex.match(f".*{regex_pattern}", _name(x)), services)
        )
    ...
    if blacklist:
        services = list(
            filter(
                lambda x: not any(
                    regex.match(f".*{pattern}", _name(x)) for pattern in blacklist
                ),
                services,
            )
        )

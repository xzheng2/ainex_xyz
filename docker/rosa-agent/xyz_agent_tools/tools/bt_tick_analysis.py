...
from xyz_agent_tools.bt_analysis.raw_tick import get_raw_tick_bundle
# JSONL schema legend lives in the shared (langchain-free) formatter module so the
# one-tick, cross-tick, and digest tools all interpret raw events the same way.
from xyz_agent_tools.bt_analysis.cross_tick_format import _JSONL_SCHEMA_LEGEND
...
_DEFAULT_PROJECT_CONTEXT = (
    "This is an Ainex behavior tree. At a high level, the robot maintains a safe "
    "standing state, uses vision/blackboard information to decide its actions, and "
    "emits ROS commands through the BT semantic/communication layers. "
...
def analyze_bt_tick(
    ...
    Args:
        ...
        project_context:   optional high-level description of the project goal and
                           expected behavior.  If empty, a default generic context
    ...
    TIP — pause/step mode gives the most accurate capture:
      This tool is best suited for DEEP analysis of a SINGLE paused or stepped
      tick: when the BT is paused (~bt/pause service) or advanced one tick at a
      time (~bt/step), the tick is frozen, its logs are complete, and "latest"
      points unambiguously at the tick the user is looking at.  If the BT is
      free-running, remind the user that pausing or stepping the BT first will
      make tick capture and analysis more accurate.  For how behavior evolved
      ACROSS multiple ticks, use cross_tick_analysis instead.
    """
    bundle = get_raw_tick_bundle(tick_id, include_neighbors)

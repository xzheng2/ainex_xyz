...
BT_LASTRUN_BT      = "bt_debug_lastrun.jsonl"
BT_LASTRUN_COMM    = "bt_ros_comm_debug_lastrun.jsonl"
STALE_WARN_SECONDS = 10.0   # rolling files are rewritten at most every flush_min_interval_secs
                            # (default 2.0 s); a small file age is normal during active sessions
...
def _path(filename: str, base_dir: str = BT_OBS_DIR) -> str:
    return os.path.join(base_dir, filename)


def _file_status(filename: str, base_dir: str = BT_OBS_DIR) -> dict:
    """Return existence and age info for a log file.

    Args:
        filename: bare filename (recent or lastrun).
        base_dir: directory holding the file. Defaults to the rosa-agent container

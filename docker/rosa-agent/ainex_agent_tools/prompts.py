...
def get_ainex_prompts() -> RobotSystemPrompts:
    return RobotSystemPrompts(
        ...
        critical_instructions=(
            ...
            "e.g. 'Tools used: get_robot_health (health.py), read_bt_obs (bt_obs.py)'. "
        ...
        about_your_capabilities=(
            ...
            "  get_bt_status          — LIVE tree snapshot: node statuses (SUCCESS/FAILURE/RUNNING), "
            "blackboard values including tick_id (BT iteration counter, 15 Hz) and "
            "camera_lost_count (consecutive camera frames without line, 30 Hz — NOT BT ticks). "
            "Use when asked about BT execution, which node is active, tree state, or tick_id.\n"
            ...
            "  analyze_bt_tick        — FIRST CHOICE for one-tick runtime fault diagnosis. "
            "Call when the user reports unexpected behavior in a specific tick, or the robot "
            "is in pause/step mode. If user_observation is provided: outputs explain_tick + "
            "compare_tick + diagnose_tick. If not: explain_only mode + asks for observation. "
            "Args: tick_id (default 'latest'), project_context, user_observation, "
            "rqt_observation, include_neighbors (default 1).\n"
            "  get_bt_tick_raw        — raw JSONL evidence for a specific tick (recent files only). "
            "Use when the user asks to see raw log data, or to inspect evidence before analysis.\n"
            "  read_bt_obs            — auto-selects file pair based on BT node state: "
            "if BT node is running → returns raw _recent JSONL (live, last 30 ticks, newest-first); "
            "if BT node is not running → returns structured _lastrun summary (full session). "
            "Use for: which node is active, per-tick decisions, condition inputs, "
            "ROS comm calls, post-run session statistics. Always use this instead of "
            "reading BT log files manually.\n"

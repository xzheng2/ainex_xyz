...
def get_ainex_prompts() -> RobotSystemPrompts:
    return RobotSystemPrompts(
        ...
        about_your_capabilities=(
            ...
            "  get_bt_status          — behavior tree state: node statuses (SUCCESS/FAILURE/RUNNING), "
            "blackboard values (robot_state, line_data, last_line_x, line_lost_count). "
            "Use when asked about BT execution, which node is active, or tree state.\n"
            "  get_walking_state      — walking controller on/off, gait params\n"
            "  get_latest_detections  — object, color, AprilTag detections\n"
            "  read_recent_ros_logs   — recent /rosout_agg messages (live, last 3s)\n"
            "  read_last_run_summary  — summarize saved log files from last roscore session "
            "(use this for: log history, last run files, what happened, session summary)\n"
            ...
            "  roslog_list (blacklisted — broken symlink; use read_last_run_summary instead)"
        ),

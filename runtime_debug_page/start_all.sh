...
#   - rosa-agent container running
#
# Services started:
#   Port 6080  noVNC (rqt via Xvfb in ainex container)
#   Port 8090  runtime debug page + ROSA PTY terminal (rosa-agent container)
#
# Open browser: http://<pi-ip>:8090

# No set -e: this script is idempotent (services may already be running).

# ── 1. noVNC stack in ainex ───────────────────────────────────────────────────
echo "[1/4] Starting Xvfb in ainex..."
docker exec -d ainex bash -c 'Xvfb :99 -screen 0 1600x900x24 &' 2>/dev/null || true

echo "[2/4] Starting rqt on DISPLAY=:99 in ainex..."
docker exec -d ainex bash -c 'sleep 1 && DISPLAY=:99 rqt &' 2>/dev/null || true

echo "[3/4] Starting x11vnc in ainex..."
...
echo "[4/4] Starting websockify/noVNC in ainex (port 6080)..."
...
docker exec rosa-agent bash -c 'pkill -f "app.py" 2>/dev/null; sleep 0.5; true' || true

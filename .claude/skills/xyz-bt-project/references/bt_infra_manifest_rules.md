...
> 参考实现：`marathon/infra/infra_manifest.py`、`fall_recovery/infra/infra_manifest.py`（`xyz_behavior` 包）
> 节点组合规范：见 `bt_observability_rules.md` Section 8 节点组合附加规则
> 最后更新：2026-04-17
...
| **business_in（业务入站）** | 传感器或外部输入 callback 收到的消息事件（输入适配） | 摄像头 line_data callback、IMU callback 收到数据 | `xyz_bt_edu/input_adapters/`（`ros_in` + `input_state`） |
...
```
xyz_bt_edu/                          # 标准节点库（package 级共享，只读 import）
  src/xyz_bt_edu/
    base_facade.py                     # XyzBTFacade 抽象接口
    base_node.py                       # XyzBTNode + L1/L2 节点基类
    behaviours/
      L1_perception/                   # 通用感知条件节点
      L2_locomotion/                   # 通用运动动作节点
    input_adapters/                    # 传感器输入适配（rospy.Subscriber 唯一合法位置）
      base_adapter.py                  # XyzInputAdapter business-in 基类
      imu_balance_state_adapter.py     # /imu → /latched/robot_state
      line_detection_adapter.py        # /object/pixel_coords → /latched/*

<bt_package>/                          # xyz_behavior
  <project_name>/
    tree/
      <project>_bt.py                  # 只做组装：import xyz_bt_edu 节点 + 项目专属节点
    behaviours/
      conditions.py                    # 项目专属 L1 条件节点（xyz_bt_edu 里无对应时才建）
      actions.py                       # 项目专属 L2+ 动作节点（xyz_bt_edu 里无对应时才建）
    semantics/
      semantic_facade.py               # 继承 XyzBTFacade；实现所有抽象方法
    infra/
      __init__.py
    ...
    log/
      ...
      bt_debug_lastrun.jsonl           ← BT 决策层日志
      bt_debug_recent.jsonl
      bt_ros_comm_debug_lastrun.jsonl  ← 通信层日志
      bt_ros_comm_debug_recent.jsonl
```

**包依赖要求**：`<bt_package>/package.xml` 必须声明：
```xml
<exec_depend>xyz_bt_edu</exec_depend>
```
...
节点组合约束（须在项目创建前确认）：
  - tree/<project>_bt.py 中优先使用 xyz_bt_edu.behaviours 标准节点
  - 项目 conditions.py 节点继承 xyz_bt_edu.base_node.XyzL1ConditionNode
  - 项目 actions.py 节点继承 xyz_bt_edu.base_node.XyzL2ActionNode
  - semantic_facade.py 继承 xyz_bt_edu.base_facade.XyzBTFacade
...
| `{{PROJECT_CLASS}}BBBridge` / BB bridge 已创建 | BB 镜像 topic 已注册 |
| `BTExecController` 已创建 | exec 控制 service 已注册（`~bt/run`, `~bt/pause`, `~bt/step`） |
...
**节点组合（新项目建立 / 扩展节点时）**
- [ ] `tree/<project>_bt.py` 是否有可替换为 `xyz_bt_edu` 标准节点的自定义节点？→ 优先使用库节点
- [ ] 新增条件节点 → `behaviours/conditions.py`；新增动作节点 → `behaviours/actions.py`？
- [ ] 新增 condition/action 是否分别继承 `XyzL1ConditionNode` / `XyzL2ActionNode`
      并声明 `LEVEL` / `BB_READS` / `BB_WRITES`？
- [ ] action 节点是否只 emit `decision` / `action_intent`，不直接 emit `ros_out`？
- [ ] `semantic_facade.py` 是否仍完整实现 `XyzBTFacade` 的所有抽象方法？→ `xyz_bt_edu` 升级后检查
- [ ] `package.xml` 中是否声明了 `<exec_depend>xyz_bt_edu</exec_depend>`？

**基础设施清单（每次修改 BT node 时）**
...
- [ ] 是否新增了 BB key 或 input adapter？→ 同步更新 `infra/bb_ros_bridge.py` topic map + `infra_manifest.py` PROJECT_INTERFACES
...
- `infra_manifest.py` 完整示例：`xyz_behavior/marathon/infra/infra_manifest.py`
- `infra_manifest.py` 简化示例：`xyz_behavior/fall_recovery/infra/infra_manifest.py`
- node 集成示例（完整）：`xyz_behavior/marathon/app/marathon_bt_node.py`，`__init__()` 末尾
- node 集成示例（简化）：`xyz_behavior/fall_recovery/app/fall_recovery_bt_node.py`
- `bb_ros_bridge.py` 参考：`xyz_behavior/marathon/infra/bb_ros_bridge.py`（6 keys）
- `bt_exec_controller.py` 参考：`xyz_behavior/fall_recovery/infra/bt_exec_controller.py`
- 节点库（标准节点）：`xyz_bt_edu/src/xyz_bt_edu/behaviours/`
- 接口协议：`xyz_bt_edu/src/xyz_bt_edu/base_facade.py`（`XyzBTFacade`）
- 节点基类：`xyz_bt_edu/src/xyz_bt_edu/base_node.py`
  （`XyzBTNode` / `XyzL1ConditionNode` / `XyzL2ActionNode`）
- 输入适配基类：`xyz_bt_edu/src/xyz_bt_edu/input_adapters/base_adapter.py`
  （`XyzInputAdapter`）
- 节点组合规范：`bt_observability_rules.md` Section 8

...
> 参考实现：`marathon/infra/infra_manifest.py`、`fall_recovery/infra/infra_manifest.py`（`ainex_behavior` 包）
...
```
ainex_bt_edu/                          # 标准节点库（package 级共享，只读 import）
  src/ainex_bt_edu/
    base_facade.py                     # AinexBTFacade 抽象接口
    base_node.py                       # AinexBTNode + L1/L2 节点基类
    behaviours/
      L1_perception/                   # 通用感知条件节点
      L2_locomotion/                   # 通用运动动作节点
    input_adapters/                    # 传感器输入适配（rospy.Subscriber 唯一合法位置）
      base_adapter.py                  # AinexInputAdapter business-in 基类
      imu_balance_state_adapter.py     # /imu → /latched/robot_state
      line_detection_adapter.py        # /object/pixel_coords → /latched/*

<bt_package>/                          # ainex_behavior
  <project_name>/
    tree/
      <project>_bt.py                  # 只做组装：import ainex_bt_edu 节点 + 项目专属节点
    behaviours/
      conditions.py                    # 项目专属 L1 条件节点（ainex_bt_edu 里无对应时才建）
      actions.py                       # 项目专属 L2+ 动作节点（ainex_bt_edu 里无对应时才建）
    semantics/
      semantic_facade.py               # 继承 AinexBTFacade；实现所有抽象方法
    infra/
      __init__.py
...
```xml
<exec_depend>ainex_bt_edu</exec_depend>
```
...
节点组合约束（须在项目创建前确认）：
  - tree/<project>_bt.py 中优先使用 ainex_bt_edu.behaviours 标准节点
  - 项目 conditions.py 节点继承 ainex_bt_edu.base_node.AinexL1ConditionNode
  - 项目 actions.py 节点继承 ainex_bt_edu.base_node.AinexL2ActionNode
...
**节点组合（新项目建立 / 扩展节点时）**
- [ ] `tree/<project>_bt.py` 是否有可替换为 `ainex_bt_edu` 标准节点的自定义节点？→ 优先使用库节点
- [ ] 新增条件节点 → `behaviours/conditions.py`；新增动作节点 → `behaviours/actions.py`？
- [ ] 新增 condition/action 是否分别继承 `AinexL1ConditionNode` / `AinexL2ActionNode`
      并声明 `LEVEL` / `BB_READS` / `BB_WRITES`？
- [ ] action 节点是否只 emit `decision` / `action_intent`，不直接 emit `ros_out`？
- [ ] `semantic_facade.py` 是否仍完整实现 `AinexBTFacade` 的所有抽象方法？→ `ainex_bt_edu` 升级后检查
- [ ] `package.xml` 中是否声明了 `<exec_depend>ainex_bt_edu</exec_depend>`？
...
- `infra_manifest.py` 完整示例：`ainex_behavior/marathon/infra/infra_manifest.py`
- `infra_manifest.py` 简化示例：`ainex_behavior/fall_recovery/infra/infra_manifest.py`
- node 集成示例（完整）：`ainex_behavior/marathon/app/marathon_bt_node.py`，`__init__()` 末尾
- node 集成示例（简化）：`ainex_behavior/fall_recovery/app/fall_recovery_bt_node.py`
- `bb_ros_bridge.py` 参考：`ainex_behavior/marathon/infra/bb_ros_bridge.py`（6 keys）
- `bt_exec_controller.py` 参考：`ainex_behavior/fall_recovery/infra/bt_exec_controller.py`
- 节点库（标准节点）：`ainex_bt_edu/src/ainex_bt_edu/behaviours/`
- 接口协议：`ainex_bt_edu/src/ainex_bt_edu/base_facade.py`（`AinexBTFacade`）
- 节点基类：`ainex_bt_edu/src/ainex_bt_edu/base_node.py`
  （`AinexBTNode` / `AinexL1ConditionNode` / `AinexL2ActionNode`）
- 输入适配基类：`ainex_bt_edu/src/ainex_bt_edu/input_adapters/base_adapter.py`
  （`AinexInputAdapter`）

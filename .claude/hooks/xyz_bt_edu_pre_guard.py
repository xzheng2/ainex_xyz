#!/usr/bin/env python3
"""PreToolUse hook: inject xyz-bt-edu-extend skill reminder before writing to xyz_bt_edu source."""
import json
import re
import sys
...
_NODE_REMINDER = (
    "【xyz_bt_edu 节点守护 · 事前提醒】\n"
    "检测到即将写入 xyz_bt_edu behaviours/L*/... BT节点文件。\n"
    "请确保内容符合 AinexBTNode 规范（xyz-bt-edu-extend skill）：\n"
    ...
    "如需模板，请先调用 xyz-bt-edu-extend skill 再写代码。"
)

_ADAPTER_REMINDER = (
    "【xyz_bt_edu 适配器守护 · 事前提醒】\n"
    "检测到即将写入 xyz_bt_edu input_adapters/... 适配器文件。\n"
    "请确保内容符合两阶段锁协议（xyz-bt-edu-extend skill）：\n"

...
- **<proj>_bt_node.py**: wired with 3rd adapter (import + `__init__` + both `run()` phases)
- **bb_ros_bridge.py**: `/latched/detected_count` → `/bt/<proj>/bb/detected_count` (count only; list too large)

...
**Log dir**: `docker/ros_ws_src/ainex_behavior/<proj>/log/` (excluded from git)

---

## Overview

Two-tier JSONL logging for the <proj> behavior tree:
...
| File | Content | Tier |
|------|---------|------|
| `<proj>/log/bt_debug_lastrun.jsonl` | BT events, full session, newest-first | last-run |
| `<proj>/log/bt_ros_comm_debug_lastrun.jsonl` | ROS comm events, full session, newest-first | last-run |
| `<proj>/log/bt_debug_recent.jsonl` | BT events, last 30 ticks, newest-first | rolling |
| `<proj>/log/bt_ros_comm_debug_recent.jsonl` | ROS comm events, last 30 ticks, newest-first | rolling |

ROSA container mount: `/home/pi/docker/ros_ws_src/ainex_behavior/<proj>/log` → `/opt/ainex_bt_log:ro`
...
Central logger. Constructed once in `<proj>_bt_node.py`.
...
## the project BT Integration Points

- **`<proj>_bt_node.py`**: constructs logger, BTDebugVisitor, attaches pre/post handlers; `self._tick_id += 1` then `self._bb.tick_id = self._tick_id` before each `tree.tick()`; `logger.close()` is no-op on shutdown
- **`<proj>_bt.py`**: `bootstrap(logger=None, tick_id_getter=None)` passes to all condition and action node constructors
...
|--------|-----------|-------------|
| `/tick_id` | `/bt/<proj>/bb/tick_id` | BT iteration counter (15 Hz, increments only when should_tick()=True) |
| `/robot_state` | `/bt/<proj>/bb/robot_state` | Stand/fall state string |
| `/line_data` | `/bt/<proj>/bb/line_data` | Current line detection object or null |
| `/last_line_x` | `/bt/<proj>/bb/last_line_x` | Last known line x position |
| `/camera_lost_count` | `/bt/<proj>/bb/camera_lost_count` | Consecutive camera frames (30 Hz) without line detection; resets to 0 on detection. NOT BT ticks. |

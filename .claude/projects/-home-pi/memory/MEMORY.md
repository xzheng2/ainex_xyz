...
- `ainex_behavior` — marathon behavior tree via py_trees (added Mar 9 2026); publishes `py_trees_msgs/BehaviourTree` on `~log/tree` for rqt_py_trees + ROSA (added Mar 25 2026)
  - **Marathon BT restructured (Apr 10 2026)**: layered package under `marathon/`; entry: `app/marathon_bt_node.py`; tree: `tree/marathon_bt.py`; actions/conditions: `behaviours/`; algorithm: `algorithms/visual_patrol.py` (pure, no manager); semantics: `semantics/semantic_facade.py`; facade: `comm/comm_facade.py`; infra: `infra/{bb_ros_bridge,bt_exec_controller,tree_publisher,infra_manifest}.py`; CI: `check_imports.py` (AST-based, **includes `app/` layer rule**: forbid `rospy.Subscriber` + `emit_comm`); imports via `rospkg.RosPack().get_path('ainex_behavior')` (not `__file__`); `ManagerProxy` removed from business path; `infra_comm_manifest_lastrun.json` written at startup
  - **input_adapters layer (Apr 16 2026)**: `ainex_bt_edu/src/ainex_bt_edu/input_adapters/` — `ImuBalanceStateAdapter` (`/imu` → `/latched/robot_state`) + `LineDetectionAdapter` (`/object/pixel_coords` → `/latched/{line_data,last_line_x,camera_lost_count}`); `rospy.Subscriber` ONLY here, forbidden in `app/` and all BT layers; **two-phase latch per tick**: `with lock: snap=adapter.snapshot_and_reset()` (atomic, all adapters together) → `adapter.write_snapshot(snap, tick_id)` (no lock, writes BB + emits `ros_in`/`input_state`); `force_state(state)` on ImuAdapter = RecoverFromFall's `robot_state_setter` hook; `ainex_bt_edu/setup.py` includes `ainex_bt_edu.input_adapters` package
